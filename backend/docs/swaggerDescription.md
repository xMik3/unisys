...
1. Call `POST /login` with one of the provided account's credentials (drop-down)
2. Copy the `token` value from the API's response
3. Click **Authorize** (top right), paste it, Authorize
4. Call the corresponding user's routes to test